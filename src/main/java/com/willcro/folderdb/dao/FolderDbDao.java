...
public class FolderDbDao {
    ...
    public Optional<FolderDbFile> getFile(String filename) throws SQLException {
        return Optional.ofNullable(q.query(connection, GET_FILE_QUERY, new BeanHandler<>(FolderDbFile.class, PROCESSOR), filename));
    }

    public Optional<FolderDbTable> getTable(String tableName) throws SQLException {
        return Optional.ofNullable(q.query(connection, GET_TABLE_QUERY, new BeanHandler<>(FolderDbTable.class, PROCESSOR), tableName));
    }

    public List<FolderDbTable> getTablesForFile(String filename) throws SQLException {
        return q.query(connection, GET_TABLES_QUERY, new BeanListHandler<>(FolderDbTable.class, PROCESSOR), filename);
    }
